                ...
                if (!availableInAncestor) {
                    ...
                    reporter.error(
                        "[Anchor DI] A parent component cannot depend on a binding that exists only in a child component. " +
                            "The binding for '$requester' (source: ${reqBinding.source}) is in component " +
                            "'${requesterComponent.substringAfterLast('.')}', " +
                            "but it depends on '$requiredType' which is only bound in child component(s): " +
                            "$onlyInChild. Parent scopes cannot see child scopes: the dependency would not be " +
                            "available when resolving '$requester'. Fix: install a module that provides " +
                            "'$requiredType' in '${requesterComponent.substringAfterLast('.')}' or an ancestor, " +
                            "or move '$requester' to a component that can see '$requiredType'.",
                        null,
                    )
                }
            }

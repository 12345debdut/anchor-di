            ...
            if (!providedKeys.contains(required) && !ValidationConstants.SKIPPED_TYPES.contains(required)) {
                ...
                reporter.error(
                    ValidationMessageFormat.formatError(
                        ...
                        fix =
                            "Add a binding for '$typeName': (1) @Inject constructor on a concrete class, " +
                                "(2) @Provides in a module installed in the same or parent component, or " +
                                "(3) @Binds in a module to map an interface to an implementation.",
                    ),
                    null,
                )
            }

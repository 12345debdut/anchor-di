...
class ScopeLifetimeViolationValidatorTest {
    ...
    fun bindsSingletonDependsOnViewModelScopedImpl_reportsError() {
        ...
        val bindings =
            listOf(
                BindingDescriptor(
                    ...
                    ValidationConstants.FQN_SINGLETON_COMPONENT,
                    ValidationConstants.FQN_SINGLETON,
                    "ProductApiBindsModule.bindProductApi",
                ),
                BindingDescriptor(
                    "com.example.ProductApiImpl",
                    null,
                    ValidationConstants.FQN_VIEW_MODEL_COMPONENT,
                    ValidationConstants.FQN_VIEW_MODEL_SCOPED,
                    "ProductApiImpl",
        ...
        ScopeLifetimeViolationValidator.validate(bindings, requirements, reporter)
        assertEquals(1, reporter.errors.size)
        val msg = reporter.errors[0].message
        assertTrue(msg.contains("longer-lived") && msg.contains("ProductApi"))
        assertTrue(msg.contains("ProductApiImpl"))
    }
}
